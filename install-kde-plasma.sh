...
-- ── Keybindings ─────────────────────────────────────────────────────
-- Ctrl+C: default behavior (SIGINT) — no override needed
-- Ctrl+V: default behavior (paste in some terminals) — we override to paste
-- Ctrl+Shift+C/V: always copy/paste (wezterm defaults, kept explicit)
config.keys = {
