#!/bin/bash
#
# install-kde-plasma-plain.sh — plain variant
...
# This is the plain variant: Breeze Dark is the default theme (no Gruvbox).
# Firefox is installed via xbps instead of Brave/Tutanota flatpaks.
#
# Usage:
#   sudo bash install-kde-plasma-plain.sh
#   sudo ./install-kde-plasma-plain.sh
#
# Optional flags:
...
#
# This plain variant uses Breeze Dark as the default theme.
#
...
# Gruvbox theme — disabled by default in this plain variant.
# Breeze Dark is the default theme. Gruvbox code is retained but skipped.
GRUVBOX=0
...
# ── pre-flight checks ────────────────────────────────────────────────
if [ "$(id -u)" -ne 0 ]; then
  echo "ERROR: Run this script as root (sudo bash install-kde-plasma-plain.sh)"
  exit 1
fi
...
echo "[*] Options: minimal=${MINIMAL} wayland=${WAYLAND} extras=${EXTRAS} firmware=${FIRMWARE} flatpak=${FLATPAK} mainline=${MAINLINE_KERNEL} btrfs_compress=${BTRFS_COMPRESS} apparmor=${APPARMOR} hardening=${HARDENING} firewall=${FIREWALL} autologin=${AUTOLOGIN:-none} reboot=${REBOOT}"
echo "[*] Logging to ${LOG}"
exec > >(tee -a "$LOG") 2>&1
echo "[*] Started: $(date)"
...
# Generated by install-kde-plasma-plain.sh
...
for user_home in /root /home/*; do
  ...
  if grep -q 'install-kde-plasma-plain.sh' "$user_bashrc" 2>/dev/null; then
...
echo ""
echo "=== Step 12.8: Creating wezterm configuration (Breeze Dark + opacity) ==="
for user_home in /root /home/*; do
...
-- ═══════════════════════════════════════════════════════════════════════
--  WezTerm Configuration — Breeze Dark with opacity
--  Generated by install-kde-plasma-plain.sh
-- ═══════════════════════════════════════════════════════════════════════
...
# kdeglobals with GruvboxPlusDark color scheme.
...
# ═══════════════════════════════════════════════════════════════════════
# Phase 13: Flatpak + Flathub + Firefox
# ═══════════════════════════════════════════════════════════════════════
echo ""
echo "══════════════════════════════════════════════════════════════"
echo "  Phase 13: Flatpak, Flathub, and Firefox"
echo "══════════════════════════════════════════════════════════════"
...
# ── Install Firefox from Void repo ─────────────────────────────────────
# Firefox is available directly from the Void Linux xbps repositories.
# We install it via xbps instead of Flatpak so it integrates natively.
echo ""
echo "=== Step 13.4: Installing Firefox ==="
xinstall firefox

# ── Update flatpak desktop database ───────────────────────────────────
echo ""
echo "=== Step 13.5: Updating desktop database ==="
command -v update-desktop-database >/dev/null 2>&1 && update-desktop-database 2>/dev/null || true
...
if [ "$GRUVBOX" -eq 1 ]; then
  ...
  if [ -z "$GRUVBOX_USER" ]; then
    ...
    echo "[*] Options: kvantum=$GRUVBOX_KVANTUM fastfetch=$GRUVBOX_FASTFETCH"
...
KDEDEFGLOBALS
    ...
    # but it overrides ColorScheme with Breeze Dark values. This
    # autostart script runs after plasmashell starts, applies the Gruvbox
    # color scheme via plasma-apply-colorscheme, then self-deletes.
...
Exec=bash -c 'sleep 5 && plasma-apply-colorscheme "SCHEME_PLACEHOLDER" 2>/dev/null; rm -f "$HOME/.config/autostart/apply-gruvbox-theme.desktop"'
...
echo "               Breeze Dark theme, 85% opacity, Nerd Font icons"
...
echo ""
echo "  Flatpak + Firefox (installed via Flathub + xbps):"
echo "    flatpak   — Flatpak runtime, Flathub remote configured"
echo "    Firefox   — installed via xbps (native Void repo package)"
